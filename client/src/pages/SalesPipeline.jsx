...
const SalesPipeline = () => {
  ...
  // ✅ FIXED: moved ABOVE useEffect usage
    ...
    return stages.map((stage) => {
      ...
      return {
        ...
        deals: filtered.map((d) => ({
          ...
          price: `$${d.dealValue || 0}`,
          company:
            d.exhibitor?.companyName ||
            d.lead?.name ||
            "Unknown",
          user: d.assignedUser?.name || "USER",
          days: getDaysAgo(d.createdAt),
          danger: false,
        })),
      };
    });
  ...
  return (
    <div className="space-y-6">
            ...
            pipeline.map((stage, index) => (
              ...
              >
                ...
                <div className="mt-5 space-y-4">
                  {stage.deals.map((deal, i) => (
                    ...
                    >
                      ...
                      <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <span className="px-2 py-1 rounded-full text-[11px] flex items-center gap-1 bg-slate-100 text-slate-500">
                          <Clock3 size={10} />
                          {deal.days}
                        </span>
                      </div>
                  ...
                  <button
                    onClick={() => navigate("/create-deal")}
                    className="w-full h-14 rounded-2xl border-2 border-slate-300 hover:border-blue-400 transition flex items-center justify-center gap-2 text-sm font-medium text-slate-600"
                  >
                    <Plus size={16} />
                    Add Deal
                  </button>
      ...
      {/* FOOTER */}
      <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 flex justify-between text-sm text-slate-500 shadow-sm">
        <p>
  ...
  return (
    <div className="flex items-start justify-between">
      <div>
        ...
        <p className={`text-xs mt-2 ${danger ? "text-red-500" : "text-slate-400"}`}>
          {subtitle}
        </p>
      </div>
